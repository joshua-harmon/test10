<template>
  <div class="py-10 px-6">
    <div class="max-w-sm mx-auto py-10 px-6 bg-gray-100 rounded-xl flex flex-col items-center">
      <h1 class="mb-16 text-2xl">Sign up</h1>
      <form action="" class="flex flex-col justify-center">
        <input type="email" placeholder="Your email address..." class="w-full mb-4 py-2 px-6 rounded-xl">
        <input type="password" placeholder="Your password..." class="w-full mb-4 py-2 px-6 rounded-xl">
        <input type="password" placeholder="Repeat password..." class="w-full mb-4 py-2 px-6 rounded-xl">
        <button class="py-2 my-8 px-6 bg-gray-400 text-white rounded-xl border-2 border-gray-400 hover:border-gray-500 duration-150">Submit</button>
      </form>
    </div>
  </div>
</template>